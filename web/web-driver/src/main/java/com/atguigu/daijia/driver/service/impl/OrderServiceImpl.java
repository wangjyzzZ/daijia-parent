    ...
    public Boolean endDrive(OrderFeeForm orderFeeForm) throws ExecutionException, InterruptedException {
        ...
        CompletableFuture<FeeRuleResponseVo> feeRuleResponseVoCompletableFuture =
                realDistanceCompletableFuture.thenApplyAsync((realDistance) -> {
                    ...
                    feeRuleRequestForm.setStartTime(finalOrderInfo.getStartServiceTime());

                    //计算司机到达代驾开始位置时间
                    //orderInfo.getArriveTime() - orderInfo.getAcceptTime()
                    // 分钟 = 毫秒 / 1000 * 60
                    Integer waitMinute =
                            Math.abs((int) ((finalOrderInfo.getArriveTime().getTime() - finalOrderInfo.getAcceptTime().getTime()) / (1000 * 60)));
                    ...
                    BigDecimal totalAmount =
                            feeRuleResponseVo.getTotalAmount().add(orderFeeForm.getTollFee())
                                    ...
                                    .add(finalOrderInfo.getFavourFee());
        ...
        CompletableFuture<Long> orderNumCompletableFuture = CompletableFuture.supplyAsync(() -> {
            String startTime = new DateTime(finalOrderInfo.getStartServiceTime()).toString("yyyy-MM-dd") + " 00:00:00";
            String endTime = new DateTime(finalOrderInfo.getStartServiceTime()).toString("yyyy-MM-dd") + " 24:00:00";
        ...
        CompletableFuture<RewardRuleResponseVo> rewardRuleResponseVoCompletableFuture =
                orderNumCompletableFuture.thenApplyAsync((orderNum) -> {
                    ...
                    rewardRuleRequestForm.setStartTime(finalOrderInfo.getStartServiceTime());
